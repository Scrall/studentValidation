...
function updateStudentTable(data) {
  ...
  data.forEach((student, index) => {
    ...
    <td>${student.document ? `<a href="/upload/${student.document}">Открыть файл</a>` : '-'}</td>
  `;  
  ...
  if (file) {
    const fileName = file.name;
      ...
      reader.onload = () => {
          ...
          socket.emit("uploadDocument", { fileData });
      };
      reader.readAsDataURL(file);
  }
}
// Слушатель для скачивания документа
socket.on("documentUploaded", ({ fileName }) => {
  console.log(`File ${fileName} uploaded successfully.`);
});
// Обработка ошибок при загрузке документа
socket.on("documentUploadError", ({ error }) => {
